...
export default class usbDfuDevice {
  ...
  constructor(dfuStatusHandler, dfuProgressHandler, dfuDisconnectHandler) {
    // Creates a null device object
    this.device = null

    // End of the flash in bytes
    this.flashEnd = 0x080fffff
    this.dfuStatusHandler = dfuStatusHandler
    this.dfuProgressHandler = dfuProgressHandler
    this.dfuDisconnectHandler = dfuDisconnectHandler
    // Page size in bytes
    this.pageSize = 0x80
  }
  ...
  async getStatus() {
    ...
    try {
      ...
      console.log(
        'Status: ' +
          Object.keys(this.dfuError)[error] +
          ' in dfu state: ' +
          Object.keys(this.dfuState)[state] +
          ', Waiting: ' +
          pollTime +
          'ms'
      )
      ...
      if (error != this.dfuError.OK || state == this.dfuState.STATE_ERROR) {
  ...
  async setFlashAndPageSizes(flashSizeStr, pageSizeStr) {
    ...
    try {
      ...
      if (isNaN(flashSize)) {
        throw (
          'Error: Flash size is invalid. It should be a hex' +
      ...
      if (isNaN(pageSize)) {
        throw (
          'Error: Page size is invalid. It should be a hex' +
      ...
      this.flashEnd = 0x0800c000 + flashSize
  ...
  async connect() {
    ...
    try {
      ...
      this.dfuStatusHandler('Connected')

      // Clear the current state. Needed after first connection or errors
      await this.clearStatus()

      // Done and return
      return Promise.resolve(this.device.serialNumber)
    } catch (error) {
      // Catch errors
      // Return the error
      this.dfuStatusHandler('Error ' + error)
      return Promise.reject(error)
    }
  }
  ...
  async erase() {
    ...
    try {
      ...
      for (let address = 0x800c000; address < this.flashEnd; address += this.pageSize) {
        // Print the erase operation to the console
        console.log(
          'Erasing ' + this.pageSize + ' bytes at 0x0' + address.toString(16).toUpperCase()
        )
        ...
        await this.device.controlTransferOut(
          ...
          arr
        ) // Holds the erase instruction and address location
        ...
        await this.getStatus()
        console.log('got status 2')
        // Work out the percentage done
        let done = (100 / (this.flashEnd - 0x800c000)) * (address - 0x800c000)
  ...
  async program(fileArr) {
    ...
    try {
      ...
      await this.device.controlTransferOut(
        ...
        new Uint8Array([0x21, 0x00, 0xc0, 0x00, 0x08])
      )
      ...
      if (totalBlocks * 2048 > this.flashEnd - 0x0800c000) {
      ...
      for (let block = 0; block < totalBlocks; block++) {
        ...
        await this.device.controlTransferOut(
          ...
          blockData
        ) // 2048 byte block of data to program
